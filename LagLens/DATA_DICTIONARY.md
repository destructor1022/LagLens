...
- `LagLens-*-ui-burst.csv`: a separate one-second trace started by Ctrl+Alt+L in the interactive desktop session. It records the foreground process, harmless message-response time, foreground CPU/RAM, compositor timing, and whether any input occurred between samples.
